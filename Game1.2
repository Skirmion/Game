...
public:
	Game();
	void setOptions(char, char, char, char);
	void run();
...
void Game::setOptions(char one, char two, char three, char four)
{
...
{
	if (key == sf::Keyboard::W or key == sf::Keyboard::S or key == sf::Keyboard::A or key == sf::Keyboard::D) direction = { 0,0 };
	if (key == sf::Keyboard::W) movingUp = isPressed, direction[1] = -1;
	else if (key == sf::Keyboard::S) movingDown = isPressed, direction[1] = 1;
	else if (key == sf::Keyboard::A) movingLeft = isPressed, direction[0] = -1;
	else if (key == sf::Keyboard::D) movingRight = isPressed, direction[0] = 1;
...
void Game::update()
{
	...
	if (shot and (direction[0] != 0 or direction[1] != 0)) { spheres[i] = Sphere(), pos = myPlayer.getPosition(); spheres[i] = { {pos.x + myPlayer.getRadius(), pos.y + myPlayer.getRadius()} , {float(100 * direction[0]), float(100 * direction[1])},  {0, 0}, 10, 255, 0, 0, 10}; i += 1;};
	if (i == 100) i = 0;
